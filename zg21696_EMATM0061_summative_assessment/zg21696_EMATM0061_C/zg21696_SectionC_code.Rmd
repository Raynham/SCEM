...
## Explore how the performance of model varies (both on train data and validation data) when the amount of training dataset changes.

After defining the hyperparameters, this exploration will require a adjusted cross-validation method 

```{r}
adjusted_splitData = function(data,folds){
  num = data%>%nrow()
...
kknn_accuracy = function(ds1,ds2, k_neighbor=10){
  ds1$diagnosis = factor(ds1$diagnosis)
  ds2$diagnosis = factor(ds2$diagnosis)
...
folds = 5
inc = 3
all_sample_list = adjusted_splitData (train_validation_data,folds)
...
## Explore how the performance of model varies (both on train data and validation data) when the hyperparameter, k,changes
about this part, we still should apply the idea for crossing validation to get the more credible accuracy changing situation as the hyperparameter k changes.
```{r}
folds = 5
all_folds_ds = adjusted_splitData (train_validation_data,folds)
...
train_validation_data$diagnosis=factor(train_validation_data$diagnosis)
...
traditional_knn = function(train,test){
  ...
  test$diagnosis = factor(test$diagnosis)
...
preds_traindtion = traditional_knn(train_validation_data,test_data)
truth = test_data$diagnosis
kd_preds_table = table(truth,preds_kdtree)
kd_preds_table
trad_preds_table = table(truth,preds_traindtion)
trad_preds_table
